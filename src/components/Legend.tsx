...
const UnifiedLegend: React.FC<UnifiedLegendProps> = ({ 
  priceSeriesData, 
  ratioSeriesData, 
  selectedDate, 
  priceValues = {}, 
  ...
  const allLegendItems: LegendItem[] = [
    ...
    { label: "TQQQ Ratio", color: blue, type: "line", seriesKey: "Ratio" },
    { label: "Portfolio Pullback", color: red, type: "line", seriesKey: "pullback" },
  ];
    ...
    <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 1, flexWrap: "wrap" }}>
      ...
      {selectedDate && (
        <Box sx={{ display: "flex", flexDirection: "column", alignItems: "flex-start", minWidth: "100px" }}>
          <Typography variant="body2" sx={{ fontSize: "12px", fontWeight: "normal", color: "text.secondary" }}>
            Date
          </Typography>
      ...
      {availableItems.map((item, index) => (
        <Box key={index} sx={{ display: "flex", flexDirection: "column", alignItems: "flex-start", minWidth: "120px" }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 0.5 }}>
            ...
            <Typography variant="body2" sx={{ fontSize: "12px", fontWeight: "normal" }}>
              {item.label}
            </Typography>
          </Box>
          {selectedDate && (
